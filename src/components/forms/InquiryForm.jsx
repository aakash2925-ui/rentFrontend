...
export default function InquiryForm({ property }) {
  ...
  const deliveryCharge = Math.ceil(deliveryDistanceKm * DELIVERY_RATE_PER_KM);
  ...
  const submit = async (event) => {
    ...
    if (deliveryDistanceKm <= 0) return setError("Enter delivery distance in km.");
    try {
  ...
  return (
    <form onSubmit={submit} className="mt-4 rounded-lg border border-stone-200 bg-white p-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
      ...
      <div className="mt-4 space-y-3">
        ...
        <div className="rounded-lg border border-meadow bg-meadow/10 p-3 text-sm font-semibold text-meadow">
          Doorstep delivery is required for this rental.
        </div>
        <div className="space-y-2 rounded-lg border border-stone-200 p-3 dark:border-stone-700">
          ...
          <input className="field" type="number" min="0" step="0.1" placeholder="Delivery distance in km" required value={form.deliveryDistanceKm} onChange={(e) => setForm({ ...form, deliveryDistanceKm: e.target.value })} />
